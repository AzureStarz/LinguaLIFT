...
class FFNWithAddedBosEos(nn.Module):
    def __init__(self, dim, out_dim):
        super().__init__()
        self.ffn = FeedForward(dim, out_dim)
        self.enc_bos = nn.Parameter(torch.randn(out_dim))
        self.enc_eos = nn.Parameter(torch.randn(out_dim))

    def forward(self, x, *args):
        x = self.ffn(x)
        bos = repeat(self.enc_bos, 'd -> b d', b=x.shape[0])
        bos = rearrange(bos, 'b d -> b 1 d')
        eos = repeat(self.enc_eos, 'd -> b d', b=x.shape[0])
        eos = rearrange(eos, 'b d -> b 1 d')
        x = torch.cat((bos, x, eos), dim=1)
        return x
