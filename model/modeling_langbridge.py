...
from .alignment_modules import LinearWithAddedEos, PerceiverResampler, FFNWithAddedEos, FFNWithAddedBosEos
...
class LBBaseModel(ABC, PreTrainedModel):
    ...
    def __init__(self, config: LangBridgeConfig, random_init=True, suppress_warnings=True):
        ...
        if config.alignments == 'linear':  # default
            ...
            self.alignment = FFNWithAddedBosEos(
                dim=config.dim_enc, out_dim=config.dim_lm)
...
class LangBridgeModel(PreTrainedModel):
    ...
    def get_encoder_features(self, enc_ids, enc_mask):
        return self.lb.get_encoder_features(enc_ids=enc_ids, enc_mask=enc_mask)
